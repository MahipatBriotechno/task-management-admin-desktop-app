...
import { logoutAdmin } from "../../../state-management/admin/login/loginActions";
import { authAdminReducer } from "../../../state-management/admin/login/loginReducer";
import { logoutUser } from "../../../state-management/user/login/action";

interface AccountDropProps {
  isAdmin: any;
}

const AccountDropdownMenu = ({ isAdmin }: AccountDropProps) => {
  ...
  return (
    <div ref={dropdownRef} className="relative inline-block text-left">
      ...
      <div className="flex items-center gap-3">
        <h3 className="capitalize text-white text-base font-light">
          Welcome <span className="font-semibold">{db?.userData?.name}</span>
        </h3>
        <button
          type="button"
          onClick={toggleMenu}
